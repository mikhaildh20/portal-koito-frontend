...
function EditContentPage() {
  ...
  const loadData = useCallback(async () => {
    try{
      ...
      const response = await fetchData(
        `${API_LINK}Content/GetContentDetail/${id}`,
        {},
        "GET"
      );
  ...
  useEffect(() => {
    if (formData.tleId && dataTitleList.length > 0) {
      const selectedTitle = dataTitleList.find(
        (item) => item.Value === Number(formData.tleId)
      );
  ...
  const handleSubmit = useCallback(
    async (e) => {
      ...
      try {
        ...
        const res = await fetchData(
          API_LINK + "Content/UpdateContent",
          payload,
          "PUT"
        );
        ...
        if (!res.error) {
          ...
          Toast.error(res.message || "Error occured while updating content.");
        }
      } catch (err) {
    ...
    [validateForm, formData, file, type, router]
  );
              ...
              <div className="col-lg-4">
                {type === "News" ? (
                    <div>
                      ...
                      />
                      {formData.contentLink && !file && (
                        <small className="text-muted">
                          Current file: {formData.contentLink.split('/').pop()}
                        </small>
                      )}
                    </div>
                  ) : (
